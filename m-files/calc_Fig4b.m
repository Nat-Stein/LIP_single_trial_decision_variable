...
load(fullfile(par_sess{1}.saveLoc, 'decoder_excl_SL'), 'excl', 'exclName', 'useN_dec', 'winCenter', 'winStart', 'winEnd')
    ...
    if length(useNs) > 0
        for i = 1 : length(winStart)
            ...
            for k = 1 : length(winStart)
                ...
                se_all_matrix(i, k) = sqrt( (pcDD_test(i)*(1-pcDD_test(i))) / sum(~train&lDD)) ;
...
save(fullfile(par_sess{1}.saveLoc, 'decoderPerf_crossTP'), 'pc_allMat', 'winStart', 'winCenter', 'winEnd')
